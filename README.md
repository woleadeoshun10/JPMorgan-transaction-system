...
│  (External) │   Query processed  └──────────────────┘
└─────────────┘   transactions


# Task 2 Kaffka
┌─────────────┐
│  Frontend   │ (Producer - creates transactions)
└──────┬──────┘
       │
       │ Sends transaction messages
       ▼
┌─────────────────────────────────┐
│         KAFKA BROKER            │
│  ┌──────────────────────────┐   │
│  │  Topic: trader-updates   │   │ ← This is your queue
│  │  [msg1][msg2][msg3][msg4]│   │
│  └──────────────────────────┘   │
└─────────────┬───────────────────┘
              │
              │ Backend reads messages when ready
              ▼
       ┌──────────────┐
       │ Midas Backend│ (Consumer - processes transactions)
       └──────┬───────┘
              │
              ▼
       ┌──────────────┐
...
# Task 4
Kafka → Consumer → Validate → Call Incentive API → Update balances + incentive → Save
Key Concept: Service-to-Service Communication
In microservices, different services talk to each other via HTTP:
┌─────────────┐           HTTP POST            ┌─────────────────┐
│ Midas Core  │ ────────────────────────────> │ Incentive API   │
...
Incentive API returns: $5 bonus
Result: Alice loses $100, Bob gains $105 ($100 + $5 bonus)
Kafka → Consumer → TransactionService
                       ↓
                   Validate (3 checks)
                       ↓
                   Call Incentive API ← External Service
                       ↓                (http://localhost:8081)
                   Get bonus amount
                       ↓
                   Update balances:
                   - Sender: -$100
                   - Recipient: +$100 + $5 (incentive)
                       ↓
                   Save to database
                       ↓
                   H2 Database
...
└──────┬──────┘
       │
       ├─── POST transactions → Kafka
       │
       └─── GET /balance?userId=X → REST API
                                        ↓
                                   ┌────────────┐
                                   │ Controller │
                                   └─────┬──────┘
...
│              MIDAS CORE (Complete!)                 │
│                                                     │
│  🌐 REST API Layer (Task 5)                        │
│     GET /balance?userId=X → JSON response          │
│                                                     │
│  🔄 External API Integration (Task 4)              │
│     Calls Incentive API → Applies bonuses          │
│                                                     │
│  💾 Database Layer (Task 3)                        │
│     JPA/Hibernate → H2 Database                    │
│     Validates & persists transactions              │
│                                                     │
│  📨 Message Queue (Task 2)                         │
